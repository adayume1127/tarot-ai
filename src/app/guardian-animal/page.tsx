...
import type { GuardianAnimal } from '@/lib/guardian-animal'

const ParticleBackground = dynamic(() => import('@/components/ParticleBackground'), { ssr: false })

type State = 'idle' | 'reading' | 'done'

export default function GuardianAnimalPage() {
  const [authChecked, setAuthChecked] = useState(false)
  const [isPremium, setIsPremium] = useState(false)
  const [birthdate, setBirthdate] = useState('')
  const [state, setState] = useState<State>('idle')
  const [reading, setReading] = useState('')
  const [animal, setAnimal] = useState<GuardianAnimal | null>(null)
  ...
  const handleSubmit = async () => {
    ...
    const res = await fetch('/api/guardian-animal', {
      method: 'POST',
    ...
    <div className="relative min-h-screen" style={{ background: 'linear-gradient(135deg, #0a0014 0%, #0d0026 100%)' }}>
      ...
      <main className="relative z-10 max-w-lg mx-auto px-6 py-6">
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center mb-6">
          <p className="font-cinzel text-xs tracking-[0.4em] text-yellow-400/50 mb-2">— GUARDIAN ANIMAL —</p>
          <h1 className="font-zen text-2xl font-bold text-white mb-1">ルナの守護動物診断</h1>
          <p className="font-zen text-purple-300/50 text-xs mt-2">生年月日からあなたを守る神秘の動物を占います</p>
        </motion.div>
          ...
          {state === 'done' && reading && animal && (
            <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1, ease: 'easeOut' }} className="mb-6">
              <div className="flex flex-col items-center mb-5">
                ...
                <div className="font-cinzel text-xs text-yellow-400/60 tracking-widest">YOUR GUARDIAN</div>
                <div className="font-zen text-yellow-300 font-bold text-xl mt-1">{animal.name}</div>
              </div>
