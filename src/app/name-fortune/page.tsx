...
import type { NameFortuneMeaning } from '@/lib/name-fortune'
...
export default function NameFortunePage() {
  ...
  const [meaning, setMeaning] = useState<NameFortuneMeaning | null>(null)
    ...
    <div className="relative min-h-screen" style={{ background: 'linear-gradient(135deg, #0a0014 0%, #0d0026 100%)' }}>
      ...
      <main className="relative z-10 max-w-lg mx-auto px-6 py-6">
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center mb-6">
          <p className="font-cinzel text-xs tracking-[0.4em] text-yellow-400/50 mb-2">— NAME FORTUNE —</p>
          <h1 className="font-zen text-2xl font-bold text-white mb-1">名前占い</h1>
          <p className="font-zen text-purple-300/50 text-xs mt-2">お名前からあなたの運気を読み解きます</p>
        </motion.div>
        ...
        {(state === 'idle' || state === 'done') && (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="mb-6">
            <label className="font-cinzel text-[10px] tracking-[0.3em] text-purple-300/40 mb-2 block text-center">NAME — お名前</label>
            <input
              type="text"
              value={name}
              onChange={(e) => setName(e.target.value)}
              placeholder="例：さくら、TARO、たろう"
              maxLength={30}
          ...
          {state === 'done' && reading && meaning && (
            <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1, ease: 'easeOut' }} className="mb-6">
              <div className="flex flex-col items-center mb-5">
                ...
                <div className="font-cinzel text-xs text-yellow-400/60 tracking-widest">NAME NUMBER {meaning.number}</div>
